...
A python script designed to build and train a neural network to output differential cross-section ratios for given input events
"""
...
from all_training_utils import build_loss, simple_deep_dense_net, float_to_string, EpochModelCheckpoint
...
def main() -> None:
    ...
    # Loading data
    PATH_TO_DATA = "/home/kye/Data/cHW_events.pkl"
    ...
    BATCH_SIZE = 1000000
    COEF_VALUES = (-5.0, -2.0, -1.0, -0.5, -0.2, -0.1, -0.05, -0.02, -0.01, 0.01, 0.02, 0.05, 0.1, 0.2, 0.5, 1.0, 2.0, 5.0)
    ...
    # Training settings
    OPTIMIZER = k.optimizers.Adam(learning_rate=k.optimizers.schedules.ExponentialDecay(
    ...
    # Checkpointing
    CHECKPOINT_PATH = "/home/kye/Checkpoints/OldLong2"
    CHECKPOINT_PERIOD = 100
    
    # Saving
    SAVE_DIRECTORY = ("/home/kye/Results/OldLong2")
    NETWORK_NAME = f"cHW_{str(HIDDEN_LAYERS).strip('()').replace(', ', 'x')}"
    
    
    # Make directory for saving and checkpointing
...
def get_data(filepath: str, 
    ...
    true = np.concatenate((sm_weights, weights), axis=1)
    
    # Remove 0s
    training_data = training_data[(true != 0).all(axis=1)]
    true = true[(true != 0).all(axis=1)]
...
def build_model(training_data: tf.Tensor, 
                ...
                activation: str, 
                dropout_frac: float, 
                optimizer: ty.Union[str, k.optimizers.Optimizer], 
    ...
    # Compile the model
    full_model.compile(optimizer=optimizer, loss=build_loss(training_coefs))
...
def train_model(model: k.models.Model, 
                ...
                true: tf.Tensor, 
                early_stopping: bool, 
                batch_size: ty.Union[int, None], 
                epochs: int, 
                patience: int, 
                min_delta: float, 
                reduce_lr_on_plateau: bool, 
                checkpoint_path: ty.Union[str, None], 
