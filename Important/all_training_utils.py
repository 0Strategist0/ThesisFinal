...
### Classes:
* EpochModelCheckpoint: A subclasse of the checkpoint callback in Keras. Lets you set checkpoints after a certain \
...
def events_to_training(events: pd.DataFrame, 
                       ...
                       int_prefix: ty.Union[str, None] = None, 
                       bsm_prefix: ty.Union[str, None] = None) -> tuple[np.ndarray, np.ndarray]:
...
def simple_deep_dense_net(input_shape: tuple, 
    ...
    for neurons in hidden_layers[1:]:
        net.add(k.layers.Dense(neurons, activation, kernel_initializer="truncated_normal"))#, kernel_regularizer=k.regularizers.L1L2(l1=1e-5, l2=1e-4)))
        if batch_normalization:
            net.add(k.layers.BatchNormalization(synchronized=True))
        if dropout_frac > 0.0:
            net.add(k.layers.Dropout(dropout_frac))
...
def build_score_loss(training_coefs: ty.Iterable, MSE_frac: float, score_subfrac: float) -> ty.Callable[[tf.Tensor, tf.Tensor], tf.Tensor]:
    ...
    if MSE_frac < 1.0:
        ...
        def loss(weights_score_quad: tf.Tensor, network_output: tf.Tensor) -> tf.Tensor:
            ...
            mse = tf.math.reduce_mean((tf.abs(weights_score_quad[:, 0, tf.newaxis]) + tf.abs(weights_score_quad[:, 1:])) 
                                      * (f_value - tf.math.log(tf.abs(weights_score_quad[:, 1:] / weights_score_quad[:, 0, tf.newaxis]))) ** 2.0, 
                                      axis=1)
